...
public class SnowstormRefset extends SnowstormAbstract {
        ...
        if (refsetEditParameters.getRefsetId() == null) {
            ...
            final ObjectNode descriptions = ThreadLocalMapper.get().createObjectNode().set("descriptions",
                ThreadLocalMapper.get().createArrayNode()
                    .add(ThreadLocalMapper.get().createObjectNode().put("moduleId", moduleId).put("term", refsetEditParameters.getName())
                        .put("typeId", "900000000000013009").put("caseSignificance", "CASE_INSENSITIVE").put("lang", "en").set("acceptabilityMap",
                            ThreadLocalMapper.get().createObjectNode().put("900000000000509007", "PREFERRED").put("900000000000508004", "PREFERRED")))
                    .add(ThreadLocalMapper.get().createObjectNode().put("moduleId", moduleId)
                        .put("term", refsetEditParameters.getName() + " (foundation metadata concept)").put("typeId", "900000000000003001")
            ...
            final ObjectNode relationships = ThreadLocalMapper.get().createObjectNode().set("relationships",
                ThreadLocalMapper.get().createArrayNode()
                    .add(ThreadLocalMapper.get().createObjectNode().put("moduleId", moduleId).put("destinationId", parentConceptId).put("typeId", "116680003")
            ...
            final ObjectNode classAxioms = ThreadLocalMapper.get().createObjectNode().set("classAxioms",
                ThreadLocalMapper.get().createArrayNode()
                    .add(ThreadLocalMapper.get().createObjectNode().put("moduleId", moduleId).put("definitionStatusId", "900000000000074008")
                        .set("relationships", ThreadLocalMapper.get().createArrayNode().add(ThreadLocalMapper.get().createObjectNode().put("moduleId", moduleId)
                            .put("destinationId", "446609009").put("typeId", "116680003").put("groupId", 0)))));
    ...
    public static String getNewRefsetId(final String editionBranchPath) throws Exception {
        ...
        if (!SnowstormBranch.doesBranchExist(projectBranchPath)) {
            SnowstormBranch.createBranch(editionBranchPath, WorkflowService.getProjectBranchName(editionBranchPath));
        }

        if (SnowstormBranch.doesBranchExist(projectBranchPath + "/" + WorkflowService.TEMP_BRANCH_NAME)) {
            tempBranchPath = projectBranchPath + "/" + WorkflowService.TEMP_BRANCH_NAME;
        } else {
            tempBranchPath = SnowstormBranch.createBranch(projectBranchPath, WorkflowService.TEMP_BRANCH_NAME);
        }
