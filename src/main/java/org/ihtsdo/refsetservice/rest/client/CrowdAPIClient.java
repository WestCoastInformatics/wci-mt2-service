...
public class CrowdAPIClient extends CrowdClientAbstract {
	...
	public static User getUser(final String userName) throws Exception {
		...
		try (final Response response = get(BASE_URL + GET_USER + "?username=" + userName);) {
			...
			if (response.getStatus() == 200) {
				...
				throw new Exception("The user " + userName + " could not be found. Received HTTP "
	...
	/**
	 * Add all groups with roles e.g. rt2-no-abc-author. - rt2 is the application -
	 * no is the two letter code for the organization (country) - abc is the acronym
	 * of the group name - author is the role (admin, author, reviewer and viewer
	 * are the others)
	 *
	...
	public static void addGroup(final String organization, final String projectName, final String projectDescription,
			final boolean generateProjectName) throws Exception {

		logger.info("Add group {} to organization {} with description of {}", projectName, organization,
				projectDescription);
		...
		final String description = (!StringUtils.isEmpty(projectDescription)) ? projectDescription.trim()
				: projectName.trim();

		/*
		 * {"name": "rt2-test-all-author", "description": "test crowd client", "type":
		 * "GROUP" }
		 */
		for (String role : ROLES) {
			...
			final String entity = "{\"name\": \"" + groupName + "\", \"description\": \"" + description
					+ "\", \"type\": \"GROUP\" }";

			logger.info("CALL CROWD API payload: " + entity);
			try (final Response response = post(BASE_URL + ADD_GROUP, entity);) {
				...
				if (response.getStatus() == 201) {
					...
					logger.error("The group " + groupName + " could not be created. Received HTTP "
							+ response.getStatus() + " from the API server.");
					throw new Exception("The group " + groupName + " could not be created. Received HTTP "
							+ response.getStatus() + " from the API server.");
				}
			}
		}
	...
	public static String addAdminGroup(final String organization, final String description) throws Exception {
		...
		/*
		 * {"name": "rt2-test-all-admin", "description": "admin for organization",
		 * "type": "GROUP" }
		 */
		...
		try (final Response response = post(BASE_URL + ADD_GROUP, entity);) {
			...
			if (response.getStatus() == 201) {
				...
				logger.error("The group " + groupName + " could not be created. Received HTTP " + response.getStatus()
						+ " from the API server.");
				throw new Exception("The group " + groupName + " could not be created. Received HTTP "
						+ response.getStatus() + " from the API server.");
			}
	...
	public static String getUserAvatar(String username) throws Exception {
		...
		try (final Response response = get(String.format(BASE_URL + GET_AVATAR_FOR_USER + username));) {
	...
	public static Set<String> getMembershipsForUser(final String username) throws Exception {
		...
		try (final Response response = get(BASE_URL + GET_DIRECT_GROUPS + username);) {
			...
			if (response.getStatus() == 200) {
				...
				throw new Exception(
						"The user " + username.trim() + " could not be found or the user is not a member of a group.");
			} else {
				throw new Exception("The user " + username.trim()
						+ " could not be found or the user is not a member of a group. Received HTTP "
						+ response.getStatus() + " from the API server.");
			}
		}
	}
	...
	public static Set<String> getMembershipsForGroup(String groupname) throws Exception {
		...
		try (final Response response = get(BASE_URL);) {
			return users;
		}
	 ...
	 * Add a user to a group.
	 * 
	 * @param groupname Name of the group from which the user membership will be
	 *                  added.
	...
	public static void addMembership(final String groupname, final String username) throws Exception {
		...
		try (final Response response = post(BASE_URL + ADD_USER_TO_GROUP + groupname.trim(), body);) {
			...
			if (response.getStatus() == 201) {
				...
				throw new Exception("Failed to add username " + username.trim() + " to group " + groupname.trim()
				...
				logger.warn("Failed to add username " + username.trim() + " to group " + groupname.trim()
						+ ". User is already a direct member of the group.");
			} else {
				throw new Exception("Failed to add username " + username.trim() + " to group " + groupname.trim()
						+ ". Received HTTP " + response.getStatus() + " from the API server.");
			}
		}
	}
	...
	public static void deleteMembership(final String groupname, final String username) throws Exception {
		...
		try (final Response response = delete(BASE_URL + REMOVE_USER_FROM_GROUP + "?groupname=" + groupname.trim()
			...
			if (response.getStatus() == 204) {
				...
				logger.info("Failed to remove username " + username.trim() + " from group " + groupname.trim()
						+ ". Group could not be found.");
			} else {
				throw new Exception("Failed to remove username " + username.trim() + " from group " + groupname.trim()
						+ ". Received HTTP " + response.getStatus() + " from the API server.");
			}
		}
	}
	...
	public static User findUserByEmail(final String email) throws Exception {
		...
		try (final Response response = get(BASE_URL + FIND_USER + email);) {
			...
			if (response.getStatus() == 200) {
				...
				throw new Exception("Failed to user with email of " + email + ". Received HTTP " + response.getStatus()
						+ " from the API server.");
			}
		}
	}
