...
public class TerminologyService implements RootService {
    ...
    public TerminologyService() throws Exception {
        ...
        if (searchHandlerMap == null) {
            ...
            logger.debug("  initialize search handler = " + searchHandlerMap.values().stream().map(f -> f.getName()).collect(Collectors.toSet()));
    ...
    public void beginTransaction() throws Exception {
        ...
        if (getTransactionPerOperation()) {
            throw new IllegalStateException("Error attempting to begin a transaction when using transactions " + "per operation mode.");
                        ...
                        for (final String sortField : sortFields) {
                            ...
                            if (s1 != null || s2 != null) {
                                ...
                                // otherwise handle via string comparison
                                    ...
                                    if (s1 != null && s1.toString().compareTo(s2.toString()) != 0) {
    ...
    /* see superclass */
    @Override
    public <T extends HasId> ResultList<String> findIds(final String query, final PfsParameter pfs, final Class<T> clazz, final String handler) throws Exception {
    ...
    /* see superclass */
    @Override
    public <T extends HasId> ResultList<T> find(final QueryParameter query, final PfsParameter pfs, final Class<T> clazz, final String handler) throws Exception {

        final ResultList<T> list = new ResultList<>();
        final int[] totalCt = new int[1];
        if (query == null) {
            list.setItems(getHandlerByName(StringUtility.isEmpty(handler) ? "DEFAULT" : handler, SearchHandler.class).getQueryResults("*:*", null, null, clazz, pfs, totalCt, getEntityManager()));
        } else {
            list.setItems(getHandlerByName(StringUtility.isEmpty(handler) ? "DEFAULT" : handler, SearchHandler.class).getQueryResults(query.getQuery(), query.getFieldedClauses(),
    ...
    /* see superclass */
    @Override
    public <T extends HasId> ResultList<String> findIds(final QueryParameter query, final PfsParameter pfs, final Class<T> clazz, final String handler) throws Exception {

        final ResultList<String> list = new ResultList<>();
        final int[] totalCt = new int[1];
        if (query == null) {
            list.setItems(getHandlerByName(StringUtility.isEmpty(handler) ? "DEFAULT" : handler, SearchHandler.class).getIdResults("*:*", null, null, clazz, pfs, totalCt, getEntityManager()));
        } else {
            list.setItems(getHandlerByName(StringUtility.isEmpty(handler) ? "DEFAULT" : handler, SearchHandler.class).getIdResults(query.getQuery(), query.getFieldedClauses(),
                query.getAdditionalClauses(), clazz, pfs, totalCt, getEntityManager()));
        }
        list.setTotal(totalCt[0]);
        list.setLimit(pfs == null ? new PfsParameter().getLimit() : pfs.getLimit());
    ...
    public <T extends HasId> int findTotal(final QueryParameter query, final PfsParameter pfs, final Class<T> clazz, final String handler) throws Exception {
        ...
        if (query == null) {
            return getHandlerByName(StringUtility.isEmpty(handler) ? "DEFAULT" : handler, SearchHandler.class).countQueryResults("*:*", null, null, clazz, pfs, getEntityManager());
        }

        return getHandlerByName(StringUtility.isEmpty(handler) ? "DEFAULT" : handler, SearchHandler.class).countQueryResults(query.getQuery(), query.getFieldedClauses(), query.getAdditionalClauses(),
            clazz, pfs, getEntityManager());
    }
    ...
    public void computeLuceneIndexes(final String indexedObjects) throws Exception {
            ...
            if (objectsToReindex.contains(key)) {
                ...
                try {
                    ...
                    searchSession.indexingPlan().execute(); // may not need
                                                            // anymore
                    searchSession.massIndexer(reindexMap.get(key)).batchSizeToLoadObjects(100).cacheMode(CacheMode.IGNORE).idFetchSize(100).threadsToLoadObjects(10).startAndWait();
        ...
        if (objectsToReindex.size() != 0) {
            throw new Exception("The following objects were specified for re-indexing, " + "but do not exist as indexed objects: " + objectsToReindex.toString());
        }
    ...
    public <T extends HasModified> T findSingle(final QueryParameter query, final Class<T> clazz, final String handler) throws Exception {
        ...
        final List<T> list = getHandlerByName(StringUtility.isEmpty(handler) ? "DEFAULT" : handler, SearchHandler.class).getQueryResults(query.getQuery(), query.getFieldedClauses(),
