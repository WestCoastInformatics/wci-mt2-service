...
public class TeamService extends BaseService {
     ...
     */
    public static Team createTeam(final TerminologyService service, final User authUser, final Team team) throws Exception {
    ...
    public static void validateTeamData(final TerminologyService service, final Team team, final boolean isNew) throws Exception {
        ...
        if (!StringUtility.isEmpty(team.getName())) {
            ...
            String query = "organizationId: " + team.getOrganizationId();

            // TODO: Make Team.name (and other names) Keyword Indexed
            // TODO: if isNew is false (which is called by three routines as false), quesry never actually gets launched.
            if (!isNew) {
                ...
                for (Team dbTeam : results.getItems()) {
                    if (dbTeam.getName().toLowerCase().equals(team.getName().toLowerCase())) {
     ...
     * @throws Exception the exception
     */
    public static Team getTeam(final User authUser, final String id, final boolean includeMembers) throws Exception {
        ...
        try (final TerminologyService service = new TerminologyService()) {
            ...
            if (includeMembers) {
                ...
                final Set<String> systemUsers = CrowdAPIClient.getCrowdSuperUsers();
            ...
            setUserRoles(authUser, team, team.getUserRoles());
     ...
     * @throws Exception the exception
     */
    public static Team updateTeam(final TerminologyService service, final User authUser, final Team team) throws Exception {

        final Team existingTeam = getTeam(authUser, team.getId(), true);
     ...
     * @throws Exception the exception
     */
    public static Team inactivateTeam(final TerminologyService service, final User authUser, final String teamId) throws Exception {

        // Find the object
        Team team = getTeam(authUser, teamId, true);
        ...
        for (final User teamMember : team.getMemberList()) {
            ...
            team = removeUserFromTeam(service, authUser, team, teamMember);
        }
    ...
    public static Team addUserToTeam(final TerminologyService service, final User user, final Team team, final String email) throws Exception {
        ...
        if (crowdUnitTestSkip == null || !"true".equalsIgnoreCase(crowdUnitTestSkip)) {
            ...
            if (TeamType.PROJECT.getText().equalsIgnoreCase(team.getType())) {
     ...
     * @throws Exception the exception
     */
    private static Team removeUserFromTeam(final User authUser, final String teamId, final String userId) throws Exception {

        try (final TerminologyService service = new TerminologyService()) {

            service.setModifiedBy(authUser.getUserName());

            final Team team = getTeam(authUser, teamId, true);
     ...
     * @throws Exception the exception
     */
    public static Team removeUserFromTeam(final TerminologyService service, final User authUser, final Team team, final User userToRemove) throws Exception {
        ...
        LOG.info("CALLING CROWD API");
        if (TeamType.PROJECT.getText().equalsIgnoreCase(updatedTeam.getType())) {
            ...
            if (projectList != null && projectList.getItems() != null) {
                for (final Project project : projectList.getItems()) {
                    ...
                    final ResultList<Team> teamsForProjectList = ProjectService.getProjectTeams(service, authUser, project.getId());

                    if (teamsForProjectList == null || teamsForProjectList.getItems() == null || teamsForProjectList.getItems().isEmpty()) {
                        continue;
                    }

                    // check if user has the same permission on another team in the project
                    // if so, do not remove the permission.
                    teamsForProjectList.getItems().removeIf(t -> t.getId().equals(team.getId()));
                    final Set<String> remainingPermissions = new HashSet<>();
                    remainingPermissions.addAll(teamsForProjectList.getItems().stream().filter(t -> t.getMembers().contains(userToRemove.getId()))
                        .flatMap(t -> t.getRoles().stream()).collect(Collectors.toSet()));

                    for (final String role : updatedTeam.getRoles()) {

                        if (remainingPermissions.contains(role)) {
                            continue;
                        }
        ...
        if (TeamType.ORGANIZATION.getText().equalsIgnoreCase(updatedTeam.getType())) {

            for (final String role : updatedTeam.getRoles()) {

                final String groupName = CrowdGroupNameAlgorithm.buildCrowdGroupName(updatedTeam.getOrganization().getCrowdId(), "all", "all", role);
                if (CrowdAPIClient.isRt2GroupName(groupName)) {
     ...
     * @throws Exception the exception
     */
    public static Team addRoleToTeam(final TerminologyService service, final User authUser, final String teamId, final String role) throws Exception {

        // find team
        final Team team = getTeam(authUser, teamId, true);
        return addRoleToTeam(service, authUser, team, role);
    }
     ...
     * @throws Exception the exception
     */
    public static Team addRoleToTeam(final TerminologyService service, final User authUser, final Team team, final String role) throws Exception {
        ...
        if (team.getRoles().contains(role.toUpperCase())) {
            ...
            final String message = "Role " + role + " is already a exists for team " + team.getId() + ".";
            LOG.info(message);
        ...
        if (crowdUnitTestSkip == null || !"true".equalsIgnoreCase(crowdUnitTestSkip)) {
            ...
            if (team.getType().equalsIgnoreCase(TeamType.ORGANIZATION.getText())) {
                ...
                final String groupName = CrowdGroupNameAlgorithm.buildCrowdGroupName(team.getOrganization().getCrowdId(), "all", "all", role);
     ...
     * @throws Exception the exception
     */
    public static Team removeRoleFromTeam(final TerminologyService service, final User authUser, final String teamId, final String role) throws Exception {
    ...
    public static boolean canUserEditTeam(final User user, final Team team) throws Exception {
        ...
        final Organization organization = team.getOrganization();
        final boolean isOrganizationAdmin = user.checkPermission(User.ROLE_ADMIN, organization.getCrowdId(), null, null);
