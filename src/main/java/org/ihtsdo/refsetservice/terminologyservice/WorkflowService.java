...
public final class WorkflowService {
	...
	public static final List<String> WORKFLOW_ACTIONS = new ArrayList<>(
			Arrays.asList(EDIT, CANCEL_EDIT, FINISH_EDIT, UPGRADE, CANCEL_UPGRADE, FINISH_UPGRADE, REQUEST_REVIEW,
					REVIEW, REJECT_REVIEW, ACCEPT_REVIEW, UNASSIGN, REQUEST_PUBLICATION, FAILS_RVF, REFSET_PUBLISHED));
	...
	public static List<String> startAllRefsetPublications(final TerminologyService service,
		...
		String query = "workflowStatus: " + READY_FOR_PUBLICATION + " AND editionShortName: "
				+ QueryParserBase.escape(editionShortName);
		...
		for (final Refset refset : results.getItems()) {
			...
			try {
				...
				final String refsetBranchPath = getRefsetBranchPath(refset.getEditionBranch(), refset.getRefsetId(),
						refset.getRefsetBranchId());

				mergeBranch(getProjectBranchPath(refset.getEditionBranch()), refsetBranchPath,
						"Updating branch to latest changes", true);

				final boolean merged = mergeRefsetIntoProjectBranch(refset.getEditionBranch(), refset.getRefsetId(),
						refset.getRefsetBranchId(), "Preparing for publication");

				if (!merged) {

					final String message = "Unable to merge refset into project branch for refset "
				...
				logger.error("Unable to merge refset into project branch for refset " + refset.getRefsetId()
		...
		try {
			...
			if (!refset.getWorkflowStatus().equals(READY_FOR_PUBLICATION)) {
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
						"Refset is not in the proper status to have publication completed " + refset.getRefsetId());
			}

			if (!refset.isLocalSet()) {
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
						"Refset can not be published because it is a local set " + refset.getRefsetId());
			}
			...
			Refset oldLatestVersionRefset = service.findSingle(
	...
	public static Refset setWorkflowStatusByAction(final TerminologyService service, final User user,
		...
		if ((currentStatus.equals(IN_EDIT)
			...
			final boolean merged = mergeEditIntoRefsetBranch(refset.getEditionBranch(), refset.getRefsetId(),
					refset.getEditBranchId(), refset.getRefsetBranchId(), notes);
			...
			if (merged) {
				...
				final String message = "Unable to merge edit into refset branch for refset " + refset.getRefsetId()
						+ " because the edit branch doesn't exist.";
				logger.error(message);
				throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, message);
			}
			...
			RefsetMemberService.clearAllMemberCaches(getEditBranchPath(refset.getEditionBranch(), refset.getRefsetId(),
					refset.getEditBranchId(), refset.getRefsetBranchId()));
			refset.setEditBranchId(null);
			restoreHistory = true;
		}
	...
	public static Refset setRefsetWorkflowStatus(final TerminologyService service, final User user, final Refset refset,
		...
		// Update an object
		service.update(refset);
		logger.info("Refset workflow status set to " + status + " for refset " + refset.getId() + ". Time: "
	...
	public static void addWorkflowHistory(final TerminologyService service, final User user, final String action,
		...
		logger.info("New workflow history entry with status " + refset.getWorkflowStatus() + " added for refset "
	...
	public static WorkflowHistory getCurrentWorkflow(final TerminologyService service, final Refset refset)
		...
		ResultList<WorkflowHistory> results = service.find("refsetId:" + QueryParserBase.escape(refset.getId()) + "",
				pfs, WorkflowHistory.class, null);
	...
	public static ResultList<WorkflowHistory> getWorkflowHistory(final TerminologyService service, final Refset refset,
		...
		ResultList<WorkflowHistory> results = service.find("refsetId:" + QueryParserBase.escape(refset.getId()) + query,
				pfs, WorkflowHistory.class, null);
	...
	public static void updateWorkflowNote(final TerminologyService service, final User user, final Refset refset,
		...
		logger.info("Note for workflow history entry with status " + refset.getWorkflowStatus() + " updated for refset "
				+ refset.getId());
	}
	 ...
	 * @throws Exception the exception
	 */
	public static boolean deleteRefsetBranch(final TerminologyService service, final User user,
			final String editionBranchPath, final String refsetId, final String branchId) throws Exception {
	...
	public static String getEditBranchPath(final String editionBranchPath, final String refsetId,
			final String editBranchId, final String refsetBranchId) throws Exception {
	 ...
	 * @throws Exception the exception
	 */
	public static String createEditBranch(final TerminologyService service, final User user,
			final String editionBranchPath, final Refset refset, final String refsetId, final String editBranchId,
			final String refsetBranchId) throws Exception {
	...
	public static boolean mergeEditIntoRefsetBranch(final String editionBranchPath, final String refsetId,
			final String editBranchId, final String refsetBranchId, final String comment) throws Exception {
	 ...
	 * @throws Exception the exception
	 */
	public static boolean deleteEditBranch(final TerminologyService service, final User user,
			final String editionBranchPath, final String refsetId, final String editBranchId,
			final String refsetBranchId) throws Exception {
		...
		if (rebase) {
			...
			try (final Response response = SnowstormConnection.postResponse(reviewUrl, body.toString())) {
				...
				if (response.getStatus() != Response.Status.OK.getStatusCode()
					...
					final String error = "Could not review merge branch " + sourceBranchPath + " into branch "
							+ targetBranchPath;
					logger.error(error);
					throw new Exception(error);
				}
		...
		try (final Response response = SnowstormConnection.postResponse(mergeUrl, body.toString())) {
			...
			try (final Response mergeInfoResponse = SnowstormConnection.getResponse(jobStatusUrl)) {
				...
				if (status.equals("FAILED")) {
					...
					logger.info("Merged branch " + sourceBranchPath + " into branch " + targetBranchPath + ". Time: "
							+ (System.currentTimeMillis() - start));
				}
			}
		}
	...
	public static void canUserPerformWorkflowAction(final User user, final Refset refset, final String action)
		...
		if (!WorkflowService.getAllowedActions(user, refset).contains(action)) {
			throw new ResponseStatusException(HttpStatus.FORBIDDEN,
					"Unsuccessful attempt to update workflow status for refset " + refset.getId() + " from status "
							+ refset.getWorkflowStatus() + " with action " + action);
		}
	}
	...
	public static void canUserEditRefset(final User user, final Refset refset) throws Exception {
		...
		if (!Arrays.asList(WorkflowService.IN_EDIT, WorkflowService.IN_UPGRADE).contains(refset.getWorkflowStatus())
				|| !user.getUserName().equals(refset.getAssignedUser())) {
			throw new ResponseStatusException(HttpStatus.FORBIDDEN,
					"Refset is not in the proper state or user does not have permission to edit.");
		}
	...
	public static void canUserPerformInDevelopmentActionsOnRefset(final User user, final Refset refset)
		...
		if (!Refset.IN_DEVELOPMENT.equals(refset.getVersionStatus())
			...
			throw new ResponseStatusException(HttpStatus.FORBIDDEN,
					"Refset is not in the proper state or user does not have permission to perform this action.");
		}
	}
}
