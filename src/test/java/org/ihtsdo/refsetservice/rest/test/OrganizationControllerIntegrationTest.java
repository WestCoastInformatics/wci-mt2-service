...
public class OrganizationControllerIntegrationTest extends BaseTest {
	...
	public void testCreate() throws Exception {
		...
		mvc.perform(post(url).content(originalOrg.toString()).contentType(MediaType.APPLICATION_XML))
				.andExpect(status().isUnsupportedMediaType()).andReturn();

		// method not found
		mvc.perform(post(url + "xyz").content(originalOrg.toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound()).andReturn();

		// not an affiliate so will not be created
		result = mvc.perform(post(url).content(originalOrg.toString()).contentType(MediaType.APPLICATION_JSON))
	...
	public void testUpdate() throws Exception {
		...
		// unsupported media type
		mvc.perform(put(url).content(newOrg.toString()).contentType(MediaType.APPLICATION_XML))
				.andExpect(status().isUnsupportedMediaType()).andReturn();

		// bad request
		mvc.perform(put(url + "xyz").content(newOrg.toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isBadRequest()).andReturn();

		// method not found
		mvc.perform(put(baseUrl + "xyz" + "/" + newOrg.getId()).content(newOrg.toString())
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound()).andReturn();

		// update
		url = baseUrl + "/" + newOrg.getId();
		result = mvc.perform(put(url).content(newOrg.toString()).contentType(MediaType.APPLICATION_JSON))
	...
	public void testFind() throws Exception {
		...
		// find by id
		result = mvc
				.perform(get(url).queryParam("query", "id:" + newOrg.getId()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
		...
		// find by name
		result = mvc.perform(
				get(url).queryParam("query", "name:" + newOrg.getName()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
	...
	public void testInactivate() throws Exception {
		...
		// inactive tests
		mvc.perform(delete(url).contentType(MediaType.APPLICATION_XML)).andExpect(status().isMethodNotAllowed())
				.andReturn();
	 ...
	 */
	private boolean compareOrganization(final Organization newOrganization, final Organization originalOrganization,
