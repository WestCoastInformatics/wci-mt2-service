...
public class ProjectControllerIntegrationTest extends BaseTest {
	...
	public void testCreate() throws Exception {
		...
		mvc.perform(post(url).content(originalProject.toString()).contentType(MediaType.APPLICATION_XML))
				.andExpect(status().isUnsupportedMediaType()).andReturn();

		// method not found
		mvc.perform(post(url + "xyz").content(originalProject.toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound()).andReturn();

		result = mvc.perform(post(url).content(originalProject.toString()).contentType(MediaType.APPLICATION_JSON))
	...
	public void testUpdate() throws Exception {
		...
		// unsupported media type
		mvc.perform(put(url).content(newProject.toString()).contentType(MediaType.APPLICATION_XML))
				.andExpect(status().isUnsupportedMediaType()).andReturn();

		// bad request
		mvc.perform(put(url + "xyz").content(newProject.toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isBadRequest()).andReturn();

		// method not found
		mvc.perform(put(baseUrl + "xyz" + "/" + newProject.getId()).content(newProject.toString())
		...
		result = mvc.perform(put(url).content(newProject.toString()).contentType(MediaType.APPLICATION_JSON))
	...
	public void testFind() throws Exception {
		...
		// find by id
		result = mvc.perform(
				get(url).queryParam("query", "id:" + newProject.getId()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
		final ResultList<Project> resultList1 = new ObjectMapper().readValue(content,
		...
		// find by name
		result = mvc.perform(
				get(url).queryParam("query", "name:" + newProject.getName()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
		final ResultList<Project> resultList2 = new ObjectMapper().readValue(content,
				(new TypeReference<ResultList<Project>>() {
					// n/a
				}));
	...
	public void testInactivate() throws Exception {
		...
		// inactive tests
		mvc.perform(delete(url).content(newProject.toString()).contentType(MediaType.APPLICATION_XML))
		...
		// fetch to validate
		result = mvc.perform(get(url).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound())
				.andReturn();
		...
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
		final ResultList<Project> resultList = new ObjectMapper().readValue(content,
				(new TypeReference<ResultList<Project>>() {
					// n/a
				}));
	 ...
	 */
	private boolean compareProjects(final Project newProject, final Project originalProject,
			final boolean nonUpdatedAttributes) {
	 ...
	 * @return true, if successful
	 */
	private boolean compareEditions(final Edition newEdition, final Edition originalEdition,
			final boolean nonUpdatedAttributes) {
