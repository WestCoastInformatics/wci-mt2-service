...
public class RefsetEditingTests extends AbstractRefsetTests {
	...
	public void setUp(final TestInfo info) throws Exception {
		...
		if (firstTimeSetup) {
			...
			setEditUtil(new EditUnitTestUtilities(getMvc(), getBaseUrl(), sdf, getWciTestingProjectId(),
					getWciTestingEditionId()));
	...
	public void testCreateFromListOnNewRefsetConcept() throws Exception {
		...
		final Map<String, String> refsetConcept = getEditUtil()
				.defineExtensionalRefsetConcept("testCreateFromListonNewRefsetConcept", memberConceptIds);
	 ...
	 * purpose // the data to create a refset from an existing concept (but can't be
	 * a // refset already in RT2) final Map<String, String> refsetExistingConcept =
	 * new HashMap<>(); refsetExistingConcept.put("refsetId", "762103008");
	 ...
	 * refsetExistingConcept.put("refsetDeleteStatus", "deleted"); // DO NOT LEAVE
	 * THIS UNCOMMENTED - for one test we will try to remove a // member that has
	 ...
	 * ObjectNode refsetExistingConceptBody = new ObjectMapper().createObjectNode()
	 * .put("refsetId", refsetExistingConcept.get("refsetId")) .put("name",
	...
	public void testCreateFromFileList() throws Exception {
		...
		final Map<String, String> refsetConcept = getEditUtil().defineExtensionalRefsetConcept("testCreateFromFileList",
				"list", MEMBER_ID_LIST_FILE_NAME, MEMBER_ID_LIST_FILE_PATH);
	...
	public void testCreateFromFileRF2() throws Exception {
		...
		final Map<String, String> refsetConcept = getEditUtil().defineExtensionalRefsetConcept("testCreateFromFileRF2",
				"rf2", MEMBER_ID_RF2_FILE_NAME, MEMBER_ID_RF2_FILE);
	...
	public void testCreateFromECL() throws Exception {
		...
		final Map<String, String> refsetConcept = getEditUtil().defineIntensionalRefsetConcept("testCreateFromECL",
				ecl);
	...
	/**
	 * Test creating, modifying, and deleting a new version of an existing refset in
	 * edit mode.
	 *
	 * @throws Exception the exception
	 */
	...
	public void testUpgradeRefset() throws Exception {
		...
		refset = getWorkflowUtil().updateWorkflow(refset, WorkflowUnitTestUtilities.AUTHOR_USER, WorkflowService.EDIT,
				"");
		refset = getWorkflowUtil().updateWorkflow(refset, WorkflowUnitTestUtilities.AUTHOR_USER,
				WorkflowService.FINISH_EDIT, "");
		...
		final UpgradeReplacementConcept upgradeReplacementConcept = upgradeInactiveConcept.getReplacementConcepts()
				.get(0);
		String replacementConceptId = upgradeReplacementConcept.getCode();
		getEditUtil().updateUpgradeConcept(newRefsetInternalId, RefsetMemberService.REPLACEMENT_ADDED,
				inactiveConceptId, replacementConceptId, null);

		// for the the first inactive concept remove the first replacement as a member
		getEditUtil().updateUpgradeConcept(newRefsetInternalId, RefsetMemberService.REPLACEMENT_REMOVED,
				inactiveConceptId, replacementConceptId, null);

		// remove the first inactive concept as a member
		getEditUtil().updateUpgradeConcept(newRefsetInternalId, RefsetMemberService.INACTIVE_REMOVED, inactiveConceptId,
				null, null);

		// re-add the first inactive concept as a member
		getEditUtil().updateUpgradeConcept(newRefsetInternalId, RefsetMemberService.INACTIVE_ADDED, inactiveConceptId,
				null, null);

		// get a concept to use as a custom replacement
		final ResultList<UpgradeReplacementConcept> replacementSearchResults = getEditUtil()
				.searchReplacementConcepts(newRefsetInternalId, "sprain");
		...
		getEditUtil().updateUpgradeConcept(newRefsetInternalId, RefsetMemberService.NEW_MANUAL_REPLACEMENT,
				inactiveConceptId, null, manualReplacementConcept);

		// for the the first inactive concept remove the manual replacement before it is
		// added as a member
		getEditUtil().updateUpgradeConcept(newRefsetInternalId, RefsetMemberService.REMOVED_MANUAL_REPLACEMENT,
				inactiveConceptId, replacementConceptId, null);
		...
		// Finish the upgrade
		refset = getWorkflowUtil().updateWorkflow(refset, WorkflowUnitTestUtilities.AUTHOR_USER,
				WorkflowService.CANCEL_UPGRADE, "");
	...
	public void testRefsetComparison() throws Exception {
		...
		activeRefset = getWorkflowUtil().updateWorkflow(activeRefset, WorkflowUnitTestUtilities.AUTHOR_USER,
				WorkflowService.EDIT, "");
		final String newActiveRefsetInternalId = activeRefset.getId();

		try {

			// get the comparison refset
			final Refset comparisonRefset = getGetUtil()
					.getRefsetFromInternalId(getRefsetWithInactiveConceptAsActiveMemberInternalId());
			...
			final RefsetMemberComparison refsetMemberComparison = getEditUtil()
					.getComparisonData(newActiveRefsetInternalId);
