...
public class RefsetWorkflowTest extends AbstractRefsetTests {
        ...
        for (final String user : users) {
            ...
            for (final String initialStatus : statuses) {
                ...
                final Map<String, String> results =
                        workflowUtil.getWorkflowActionPaths(user, initialStatus);

                for (final String action : results.keySet()) {

                    if (workflowUtil.getPermissiblePaths().containsKey(userRole)
                            && workflowUtil.getPermissiblePaths().get(userRole)
                                    .containsKey(initialStatus)
                            && workflowUtil.getPermissiblePaths().get(userRole).get(initialStatus)
                                    .containsKey(action)) {

                        assertThat(results.get(action)).isEqualTo(workflowUtil.getPermissiblePaths()
                                .get(userRole).get(initialStatus).get(action));
                    } else {
    ...
    public void testWorkflowHistory() throws Exception {
        ...
        final String newRefsetVersionInternalId =
                editUtil.createNewRefsetVersion(MAIN_TESTING_REFSET_ID);
        int actionCount = 1;
        ...
        updatedRefset = workflowUtil.updateWorkflow(refset, AUTHOR_USER,
                WorkflowService.REQUEST_REVIEW, note);
        ...
        workflowUtil.validateRow(lookedUpWorkflowHistory.get(actionCount), AUTHOR_USER,
        ...
        note = "";
        updatedRefset =
                workflowUtil.updateWorkflow(refset, REVIEWER_USER, WorkflowService.REVIEW, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        workflowUtil.validateRow(lookedUpWorkflowHistory.get(actionCount), REVIEWER_USER,
                WorkflowService.REVIEW, WorkflowService.IN_REVIEW, note);

        // Reject Review (with Note)
        actionCount++;
        note = "I rejected this because it's not right";
        updatedRefset = workflowUtil.updateWorkflow(refset, REVIEWER_USER,
                WorkflowService.REJECT_REVIEW, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        ...
        note = "";
        updatedRefset =
                workflowUtil.updateWorkflow(refset, AUTHOR_USER, WorkflowService.EDIT, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        workflowUtil.validateRow(lookedUpWorkflowHistory.get(actionCount), AUTHOR_USER,
                WorkflowService.EDIT, WorkflowService.IN_EDIT, note);
        ...
        updatedRefset =
                workflowUtil.updateWorkflow(refset, REVIEWER_USER, WorkflowService.WITHDRAW, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        ...
        updatedRefset = workflowUtil.updateWorkflow(refset, REVIEWER_USER,
                WorkflowService.ACCEPT_REVIEW, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        ...
        updatedRefset = workflowUtil.updateWorkflow(refset, AUTHOR_USER,
                WorkflowService.REQUEST_PUBLICATION, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        ...
        updatedRefset =
                workflowUtil.updateWorkflow(refset, AUTHOR_USER, WorkflowService.FAILS_RVF, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        ...
        updatedRefset =
                workflowUtil.updateWorkflow(refset, AUTHOR_USER, WorkflowService.CANCEL_EDIT, note);
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
        lookedUpWorkflowHistory = workflowUtil.getWorkflowHistory(newRefsetVersionInternalId);
        assertThat(lookedUpWorkflowHistory.size()).isEqualTo(actionCount + 1);
        ...
        updatedRefset = workflowUtil.updateWorkflow(refset, AUTHOR_USER,
                WorkflowService.REQUEST_PUBLICATION, "");
        assertThat(updatedRefset).isNotNull();
        refset = updatedRefset;
