...
public class TeamControllerIntegrationTest extends BaseTest {
	...
	public void testCreate() throws Exception {
		...
		mvc.perform(post(url).content(originalTeam.toString()).contentType(MediaType.APPLICATION_XML))
				.andExpect(status().isUnsupportedMediaType()).andReturn();

		// method not found
		mvc.perform(post(url + "xyz").content(originalTeam.toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound()).andReturn();

		result = mvc.perform(post(url).content(originalTeam.toString()).contentType(MediaType.APPLICATION_JSON))
	...
	public void testUpdate() throws Exception {
		...
		// unsupported media type
		mvc.perform(put(url).content(newTeam.toString()).contentType(MediaType.APPLICATION_XML))
				.andExpect(status().isUnsupportedMediaType()).andReturn();

		// bad request
		mvc.perform(put(url + "xyz").content(newTeam.toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isBadRequest()).andReturn();

		// method not found
		mvc.perform(put(baseUrl + "xyz" + "/" + newTeam.getId()).content(newTeam.toString())
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound()).andReturn();

		// update
		url = baseUrl + "/" + newTeam.getId();
		result = mvc.perform(put(url).content(newTeam.toString()).contentType(MediaType.APPLICATION_JSON))
	...
	public void testFind() throws Exception {
		...
		// find by id
		result = mvc
				.perform(get(url).queryParam("query", "id:" + newTeam.getId()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
		final ResultList<Team> resultList1 = new ObjectMapper().readValue(content,
				(new TypeReference<ResultList<Team>>() {
					// n/a
				}));
		...
		// find by name
		result = mvc.perform(
				get(url).queryParam("query", "name:" + newTeam.getName()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
		final ResultList<Team> resultList2 = new ObjectMapper().readValue(content,
				(new TypeReference<ResultList<Team>>() {
					// n/a
				}));
	...
	public void testInactivate() throws Exception {
		...
		// inactive tests
		mvc.perform(delete(url).content(newTeam.toString()).contentType(MediaType.APPLICATION_XML))
		...
		content = result.getResponse().getContentAsString();
		LOG.info(" content = {}", content);
		final ResultList<Team> resultList = new ObjectMapper().readValue(content,
				(new TypeReference<ResultList<Team>>() {
					// n/a
				}));
	 ...
	 * @return true, if successful
	 */
	private boolean compareOrganizations(final Organization newOrganization, final Organization originalOrganization,
